...
// frontend/src/app/pages/template-editor/template-editor.component.ts
...
export class TemplateEditorComponent implements AfterViewInit {
  canvas: fabric.Canvas | null = null;
  // You should make these dynamic with dropdowns in a real app
  companyId = 1; 
  bankId = 1;    
  background = '/assets/template-blank.jpg'; // Path to your blank cheque image
  ...
  ngAfterViewInit() {
    ...
    fabric.Image.fromURL(this.background, { crossOrigin: 'anonymous' }, (img: fabric.Image) => {
      if (!this.canvas) return;
      img.scaleToWidth(this.canvas.getWidth());
      img.scaleToHeight(this.canvas.getHeight());
      this.canvas.backgroundImage = img;
    ...
    // --- Important for the Direct Overlay Workflow ---
    // We only NEED to define the coordinates for the fields we are adding from the database.
    ['chequeNumber', 'micr'].forEach((field, index) => {
      const text = new fabric.Text(field.toUpperCase(), {
        left: 100,
        top: 100 + index * 40,
        fill: 'red',
        fontSize: 14,
